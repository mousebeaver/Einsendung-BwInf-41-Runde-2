...
#ifndef EVALPEUW_H_INCLUDED
#define EVALPEUW_H_INCLUDED
...
{
    /*Diese Funktion berechnet alle Pfannkuchenstapel, die durch eine PEUW-Operation nur in Stapel ueberfuehrt werden koennen,
    ...
    */
    ...
    for(vs v : source) //Durchgehen aller Pfannkuchenstapel in source
    {
        ...
        //Durchgehen aller Stapel, die durch eine PEUW-Operation in v ueberfuehrt werden koennen
        for(int pos = 0; pos <= pcounter; pos++) //Durchgehen der Position, an der ein neuer Pfannkuchen eingefuegt wird
        {
            for(short val = 1; val <= pcounter+1; val++) //Wert des neuen Pfannkuchens
            {
                ...
                if(moves[test.size()-1].count(testInd) == 0 && !sorted(test))
                {
                    ...
                    for(int k = 0; k <= pcounter; k++) //Durchgehen aller Positionen fuer die PEUW-Operation
                    {
                        vs flipped = flipOperation(test, k); //Durchfuehren einer PEUW-Operation
                        ...
                        {
                            /*Der Stapel laesst sich durch eine PEUW-Operation in einen Stapel ueberfuehren,
...
void evalP(vector<ll>& faculty) //Berechnung der Funktion P
{
    ...
    cout<<"Berechnung von PR(k) fuer k = 1, 2, ..., n\n";
    cout<<"Wert von n: ";
    cin>>n;
    ...
    for(int i = 1; i < n; i++)
    {
        ...
        cout<<"PR("<<i+1<<") = "<<output[i]<<endl; //Kontrollausgabe an die Konsole
    }

    //Ausgabe an die Ausgabedatei:
    for(int i = 0; i < n; i++)
    {
        fout<<"Es gilt PR("<<i+1<<") = "<<output[i]<<"\n";
        fout<<"Ein beispielhafter Pfannkuchenstapel S mit A(S) = PR("<<i+1<<") = "<<output[i]<<" ist:\n";
