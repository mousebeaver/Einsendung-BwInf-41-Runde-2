...
void outputPWUESequence(ofstream& fout, unordered_map<ll, vector<pil>>& backtracking, vs pancakes, vs start, vector<ll>& faculty)
{
    ...
    //Ausgabe:
    fout<<"Fuer das Sortieren des Stapels "<<stackOutput(start)<<" werden mindestens "<<output.size()-1<<" PWUE-Operationen benoetigt.\n";
    ...
    while(output.size())
    {
        ...
        vs test = pancakes; //Zum Durchfuehren von PWUE-Operationen
        ...
        {
            range++;
            //Ausfuehren der naechsten PWUE-Operation:
            test = pancakes;
            reverse(test.begin(), test.begin()+range+1);
            test.erase(test.begin());
            outputSimplify(test);
        }
        output.erase(output.begin());

        fout<<"\nNach Anwenden der PWUE-Operation auf die ersten "<<range+1<<" Pfannkuchen:\n";
        reverse(pancakes.begin(), pancakes.begin()+range+1); //Durchfuehren einer PWUE-Operation OHNE Simplifizieren
...
void evalA(vector<ll>& faculty) //Die Funktion fuer das Berechnend der Funktion A(S)
{
    ...
    unordered_map<ll, vector<pil>> backtracking; //Der Stapel mit (j+1) Pfannkuchen und Index i kann vom Stapel in backtracking[i][j] (Anzahl, Index) durch eine PWUE-Operation erreicht werden
    ...
    while(true) //Berechnung durchfuehren bis zum Erreichen einer Loesung
    {
        ...
        for(int i = 0; i < (int) current.size(); i++) //Durchgehen aller moeglichen Positionen fuer eine PWUE-Operation
        {
            ...
            if(test.empty())
            {
                ...
                outputPWUESequence(fout, backtracking, current, pancakes, faculty);
                exit(0);
            }
